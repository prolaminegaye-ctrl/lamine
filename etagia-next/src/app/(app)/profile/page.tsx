...
export default async function ProfilePage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6" style={{ color: 'var(--text)' }}>Mon profil</h1>
        ...
        <div className="space-y-3">
          {(enrollments || []).map((e: any) => (
            <div key={e.id} className="card p-4 flex items-center justify-between">
              <div>
                <div className="font-medium text-sm" style={{ color: 'var(--text)' }}>{e.course?.title}</div>
                <div className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
                  Inscrit le {new Date(e.enrolled_at).toLocaleDateString('fr-FR')}
                </div>
              </div>
              <div className="text-right">
                <div className="text-sm font-semibold" style={{ color: 'var(--accent)' }}>{e.progress_percent || 0}%</div>
          ...
          {(!enrollments || enrollments.length === 0) && (
            <p style={{ color: 'var(--text-muted)' }}>Aucun cours suivi pour l&apos;instant.</p>
          )}
        </div>
      </div>
    </div>
